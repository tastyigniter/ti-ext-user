<div class="row">
    <div class="col-md-6">
        <h3><?= sprintf(lang('main::account.text_welcome'), $customerDetails['first_name']); ?></h3>
        <?php if (count($customerDetails->address)) {
            dd($customerDetails->address); ?>
            <p>
                <a
                    class="edit-address pull-right"
                    href="<?= site_url('account/address/edit/'.$customerDetails->address->getKey()); ?>"
                    data-original-title=""
                    title=""
            ...
            <address class="text-left text-overflow"><?= format_address($customerDetails->address->toArray()); ?></address>
        <?php }
        else { ?>
    ...
    <div class="col-md-6">
        <div class="text-center">
            ...
            <?php if (count($cartItems) > 0) { ?>
                <p><?= sprintf(lang('main::account.text_cart_summary'), $cartItems, $cartTotal); ?></p>
            ...
            <?php }
                ...
                <a class="btn btn-primary" href="<?= restaurant_url('menus'); ?>">
            ...
            <div class="table-responsive">
                <table class="table table-none">
                    ...
                    <?php foreach ($orders as $order) { ?>
                        <tr>
                            <td>
                                <a href="<?= site_url('account/orders/view/'.$order['order_id']); ?>">
                                    <?= $order['order_id']; ?>
                                </a>
                            </td>
                            <td width="80%"
                            ...
                            <td>
                                <?= mdate(setting('time_format', '%h:%i %a'), strtotime($order['order_time'])); ?> -
                                <?= day_elapsed($order['order_date']); ?>
                            </td>
                        </tr>
                    <?php } ?>
                    </tbody>
            ...
            <div class="table-responsive">
                <table class="table table-none">
                    ...
                    <?php foreach ($reservations as $reservation) { ?>
                        <tr>
                            <td>
                                <a href="<?= site_url('account/reservations/view/'.$reservation['reservation_id']); ?>">
                                    <?= $reservation['reservation_id']; ?>
                                </a>
                            </td>
                            <td><?= $reservation['status_name']; ?></td>
                            <td>
                                <?= mdate(setting('time_format', '%h:%i %a'), strtotime($reservation['reserve_time'])); ?> -
                                <?= day_elapsed($reservation['reserve_date']); ?>
                            </td>
                        </tr>
                    <?php } ?>
                    </tbody>
                ...
                <table class="table table-none">
                    ...
                    <?php foreach ($messages as $message) { ?>
                        <tr class="<?= ($message['state'] == '0') ? 'unread' : 'read'; ?>">
                            <td>
                                <a class="edit"
                                   href="<?= site_url('account/inbox/view/'.$message['message_id']); ?>"
                                >
                                    <?= $message['subject']; ?>
                                </a><br/>
